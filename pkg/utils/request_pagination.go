...
type PaginationMeta struct {
	Limit  int   `json:"limit" groups:"public"`
	Offset int   `json:"offset" groups:"public"`
	Total  int64 `json:"total" groups:"public"`
}
...
func CopyKey(conditions map[string][]string, original string, replaceBy string) {
	if targetValue, ok := conditions[original]; ok {
		// delete(conditions, original)
		conditions[replaceBy] = targetValue
	}
...
func DeleteKey(conditions map[string][]string, original string) {
	// if _, ok := conditions[original]; ok {
	delete(conditions, original)
	// conditions[replaceBy] = targetValue
	// }
}
